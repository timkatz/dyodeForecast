# DYODE 2025 Recap & 2026 Strategy
## Executive Summary for Management Team

**Prepared:** November 2025
**Audience:** Executive Management Team
**Confidentiality:** Management-level only — See "Information Sharing Guidelines" section for town hall guidance

---

## Important Context: Financial Transparency

**This document represents a new level of financial transparency for the management team.**

Historically, we've shared limited financial metrics with the team:
- Profit margin % (entire company)
- % to MRR goal (progress toward profitability targets)

This summary includes **full revenue details, MRR figures, and strategic financial planning** that have not previously been shared broadly.

**Why we're sharing this now:**
- The management team needs complete visibility to lead effectively
- 2026 requires coordinated execution across all departments
- You need to understand the full picture to make informed decisions

**What this means:**
- Use this information to inform your team leadership
- Follow the "Information Sharing Guidelines" section for what to share at town halls
- Keep specific revenue/MRR figures confidential outside of management meetings

---

## Opening Message

**Team,**

2025 was a tough year. We're down in revenue, sitting at $3.6M (accrual) vs. $3.9M in 2024. After three years of decline from our $4.88M peak, we're at a critical juncture. The decline rate slowed (from -20% to -6.5%), showing progress, but we're still not growing.

**But here's the good news:** We have a clear, executable plan for 2026 that will reverse the trend.

This document outlines:
- Where we are and how we got here
- Our 2026 growth strategy
- Your role in making it happen
- The resources and support you'll have

**The bottom line:** If we execute with focus and urgency, we can hit $4.2M in 2026 and get back on a growth trajectory. It won't be easy, but it's absolutely achievable.

Let's make 2026 our turnaround year.

— Tim & [Co-Founder Name]

---

## 2025: The Reality Check

### The Numbers

**Current State (November 2025):**
- MRR: $292,946
- Active Clients: 30 unique clients
- Projected 2025 Revenue: $3.6M (accrual) / $3.4M (cash)

**3-Year Decline:**
- 2022: $4.88M (peak) - 8.5% profit margin
- 2023: $4.86M (-0.6%) - 10.8% profit margin
- 2024: $3.87M (-20%) - 5.6% profit margin
- 2025: $3.6M (-6.5%) - 6.0% profit margin (accrual)

**We've lost $1.28M in revenue over 3 years, but the decline slowed significantly in 2025.**

**Important Note:** Accrual revenue ($3.6M) reflects what clients owe us and matches our MRR tracking. Cash revenue ($3.4M) is ~$165k lower due to outstanding AR from 2 distressed clients with equity/relationship situations (not typical collections issues).

---

### What Went Wrong

**1. Churn is Killing Us**
- Current churn rate: 38-40% (20 of 53 unique clients lost)
- Industry benchmark: 15-20%
- **We're churning clients at 2x-2.5x the normal rate**

**2025 Churn:**
- Unique clients added: 13 | Unique clients lost: 20 | **Net: -7 clients**
- **Service-level impact:** Development -$25.1k MRR, Marketing -$65.4k MRR, Consulting -$24k MRR, Projects -$224.5k
- **Total retainer MRR lost: ~$114.5k** | **Total 2025 service loss: ~$339k**
- **Multi-service client retention: 100%** (only 1 of 4 multi-service clients experienced any loss)
- **See cofounders summary for detailed client-by-client churn breakdown**

**2. No Systematic Lead Generation**
- Relied on organic/referrals only until Nov 2025
- No predictable pipeline
- Feast or famine cycles

**3. Too Many Month-to-Month Contracts**
- 24% of MRR ($67k) has no contract protection
- Clients can leave anytime
- No predictability

**4. Project Dependency**
- 26% of revenue from projects
- Projects naturally end
- Not sustainable

**5. Economic Headwinds**
- Client budget freezes
- Tariffs and inflation
- Competitive pressure

---

### What We Did Right

**Launched Martal Partnership** (Nov 2025) - First systematic lead gen with 18 guaranteed SQLs
**Maintained Key Relationships** - AG Jeans, Great Star Tools, Glenmorangie still active
**Stabilized MRR** - Held at $290k in Q3-Q4, stopped the decline (temporarily)

---

## 2026: The Turnaround Plan

### The Goal

**Target:** $4.2M revenue, $381k MRR by end of 2026

**What This Means:**
- 46% MRR growth needed
- Add $209k in new MRR
- Replace 100% of churned clients PLUS grow
- Reverse 3-year decline trend

**This is ambitious but achievable.**

---

### The Strategy: Four Pillars

We're not relying on one silver bullet. We're building a diversified growth engine with four key programs:

---

#### **Pillar 1: Martal Lead Generation (External)**

**What:** Outsourced B2B lead gen partnership
**Status:** Pilot launched Nov 2025 (through Jan 2026)

**How It Works:**
- Martal does cold outreach to our ICP
- Delivers Sales Qualified Leads (SQLs)
- We close them

**2026 Target:**
- 8-10 new clients
- $60-75k MRR
- Close rate: 30-40%

**Your Role:**
- Sales team: Convert SQLs to clients
- Ops team: Support smooth onboarding
- Delivery teams: Ensure quality for retention

**Investment:** $18k/quarter (~$72k/year)
**Decision Point:** Jan 2026 (renew or pivot based on pilot results)

---

#### **Pillar 2: Client Referral Program (External)**

**What:** Clients earn 5-10% of first contract value in credits for referring new business

**Why This Matters:**
- Referrals have 60-80% close rate (vs. 30% other channels)
- Lowest CAC: $500-2k (vs. $5k+ Martal)
- Fastest sales cycle: 15-30 days
- Better client fit (referred by happy clients)

**How It Works:**
1. Client refers a brand
2. We close the deal
3. Client earns 5-10% credit (tiered by deal size)
4. Credit applies to future invoices

**2026 Target:**
- 5-8 referrals converting to clients
- $250-400k revenue impact
- Investment: $20-35k in credits

**Your Role:**
- Account managers: Ask for referrals in QBRs
- Sales team: Close referred leads quickly
- Marketing: Create referral materials

**Launch:** Jan 2026

---

#### **Pillar 3: Cross-Sell Incentive Program (Internal)**

**What:** Account managers earn 5% commission on contract value for cross-selling additional services to existing clients

**The Opportunity:**
- 26 of 30 active clients (87%) only use ONE service
- 4 multi-service clients currently (13%)
- ~20-21 viable cross-sell targets (after excluding past service history)

**Proven Value (2025 Data):**
- **Multi-service client loss rate: 0%** (0 of 4 lost completely)
- **Single-service client loss rate: 43%** (20 of ~46 lost completely)
- **Multi-service clients = 3.6x higher value** ($34k avg vs. $9.5k)
- **Real example:** Tipsy Elves lost Consulting but kept Development ($7.5k/mo retained)
- **Multi-service clients are dramatically stickier** (retention rate 100% vs. 43% for single-service)

**How It Works:**
1. AM identifies cross-sell opportunity
2. AM pitches additional service (marketing, dev, consulting)
3. Client signs contract
4. AM earns 5% commission (50% at signing, 50% after 90 days)

**Example:**
- Cross-sell marketing to dev client: $7,500/month x 6-month contract
- AM earns: $2,250 commission

**2026 Target:**
- 6-8 cross-sells
- $40-58k new MRR
- Investment: $10-16k in commissions

**Your Role:**
- Account managers: Identify and pitch opportunities (PRIMARY)
- Delivery teams: Ensure quality cross-sell delivery
- Sales team: Support proposal/closing process

**Launch:** Jan 2026

---

#### **Pillar 4: Clutch Review Incentive (External)**

**What:** Clients earn $500 credit for leaving qualified Clutch review

**Why This Matters:**
- Clutch reviews drive 30-40% of inbound leads
- Social proof critical for Shopify Plus prospects
- Long-term asset (reviews stay live for years)

**How It Works:**
1. Client leaves 4+ star review on Clutch (100+ words, specific examples)
2. Review goes live
3. Client earns $500 credit on next invoice

**2026 Target:**
- 10-15 reviews
- Improved Clutch rating and visibility
- Investment: $5-7.5k in credits

**Your Role:**
- Account managers: Ask happy clients for reviews (PRIMARY)
- Marketing: Track reviews and manage Clutch profile
- Delivery teams: Create review-worthy results

**Launch:** Jan 2026

---

#### **Strategic Priority: Maintain Shopify Plus Premier Partner Status**

**Current Status:** ⚠️ **AT RISK** - Only 1 Plus migration in past 2 years

**Why This Matters:**
- 61% of our MRR ($178k/month) comes from Shopify/Plus clients
- Partner badge drives inbound leads and credibility
- Without Premier status: Lose competitive edge and co-marketing opportunities in 2027+

**What's Required:** We must meet **ALL THREE** criteria to maintain Premier tier:

| Criterion | Requirement | Current Status | 2026 Goal |
|-----------|-------------|----------------|-----------|
| **1. Merchant Revenue** | $2M-$10M | ✅ $200M-$1B+ | ✅ Already exceeds |
| **2. Plus Migrations** | 4 (2-year rolling) | ⚠️ Only 1 | Need 3+ more in 2026 |
| **3. Team Certifications** | 30 Shopify certs | ⚠️ 12 completed, 16 in-progress | Complete in-progress + 2-4 more |

**2026 Migration Targets (To Meet Criterion 2):**

| Quarter | Target | Cumulative | Project Revenue |
|---------|--------|------------|-----------------|
| Q1 | 1 migration | 1 | $75k |
| Q2 | 2 migrations | 3 | $150k |
| Q3 | 1 migration | 4 ✅ Premier | $75k |
| Q4 | 0-2 migrations | 4-6 | $0-150k |

**Current Pipeline:**
- Jerome's Furniture: ~$75k (Q1, re-engaging)
- Bolle Brands: ~$100-150k (Q2, rebuilding relationship)
- **Critical:** Martal must deliver 10-12 migration SQLs (not just general Shopify Plus leads)

**Platform Targets (What Counts as Migration):**
- ✅ BigCommerce → Shopify Plus
- ✅ Magento/Adobe Commerce → Shopify Plus
- ✅ WooCommerce → Shopify Plus
- ✅ Custom platforms → Shopify Plus
- ❌ Existing Shopify/Plus merchants (don't count)

---

**Certification Action Plan (To Meet Criterion 3):**

**Your Team Has Already Been Assigned Courses - We Just Need Completion!**

**Current Status:**
- ✅ **12 certifications completed** (40% to goal)
- ⚠️ **16 certifications in-progress** (assigned but not completed)
- 📊 **28 potential** (if all in-progress completed)
- **Gap: Just 2-4 more certifications needed to reach 30**

**Q1 2026 Action (Jan-Mar):**

All team members with assigned Shopify courses: **Complete your in-progress certifications**

| Team | Course | People | Priority |
|------|--------|--------|----------|
| **Dev Team** | Liquid Storefronts | 5 remaining | **HIGH** |
| **PM/Sales Team** | Selling Fundamentals | 4 remaining | **HIGH** |
| **Marketing Team** | Marketing Fundamentals | 3 (Lucas, Bruce, Jessica) | **MEDIUM** |
| **Marketing Team** | Selling/Positioning Marketing | 2 team members | **MEDIUM** |
| **PM Team** | Solution Planning | 1 (Elana) | **MEDIUM** |

**Time Required:** 4-6 hours per certification (courses are free)

**Q1 Target:** Complete 10-13 in-progress → 22-25 total certifications by March 31

**Q2 Target:** Complete remaining in-progress + co-founders/directors obtain 2-4 more → 30+ total certifications by June 30 ✅

**Why Both Matter:**
Even if we close 4 Plus migrations, we can still lose Premier tier if we don't reach 30 certifications by June 30, 2026. **Both requirements must be met.**

**Your Role:**
- **Sales Team:** Prioritize migration opportunities from Martal (BigCommerce, Magento, WooCommerce)
- **All Team Members with Assigned Courses:** Complete your Shopify certifications in Q1-Q2
- **Account Managers:** When prospecting/referring, prioritize platform migration opportunities
- **Management:** Track quarterly progress on both migrations closed and certifications completed

**Tracking:** Monthly updates on migration pipeline and certification completion

---

### The Math: How We Hit $4.2M

| Channel | New Clients | MRR Impact | Investment | Your Role |
|---------|-------------|------------|------------|-----------|
| **Martal** | 8-10 | $60-75k | $72k | Sales closes |
| **Referrals** | 5-8 | $37-60k | $20-35k | AMs ask |
| **Cross-Sells** | 6-8 (expansions) | $40-58k | $10-16k | AMs pitch |
| **Founder-led** | 6-8 | $45-60k | Time | Co-founders |
| **Inbound (Clutch)** | 2-3 | $15-22k | $5-7.5k | Marketing + AMs ask for reviews |
| **TOTAL** | **27-37** | **$197-275k** | **$107-130k** | **Everyone** |

**After accounting for churn (~$112k), we net $85-163k growth → $377-435k MRR**

---

## Your Role in 2026 Success

### Account Management Team

**Primary Responsibilities:**
1. **Cross-Sell Execution**
   - Identify top 3 cross-sell opportunities per AM
   - Pitch additional services
   - Close deals
   - **Earn commissions** (this is YOUR opportunity!)

2. **Referral Generation**
   - Ask for referrals in every QBR
   - Identify well-connected clients
   - Track referrals in CRM

3. **Review Collection**
   - Ask happy clients for Clutch reviews
   - Follow up until published
   - Track in CRM

4. **Retention**
   - Conduct QBRs for all $10k+ clients
   - Proactively manage renewals (90 days out)
   - Monitor client health
   - Convert month-to-month to contracts

**Success Metrics:**
- Cross-sells pitched: 3-5 per AM
- Cross-sells closed: 1-2 per AM
- Referrals generated: 2-3 per AM
- Reviews collected: 2-3 per AM
- Churn rate: <25% for your clients

**What's In It For You:**
- Earn $5-10k in cross-sell commissions
- Top performers: $15k+ potential
- Alternative to raises during salary freeze
- Career growth opportunity

---

### Sales Team

**Primary Responsibilities:**
1. **Convert Martal SQLs**
   - Close 30-40% of qualified leads
   - Fast follow-up (24-48 hours)
   - Track in CRM

2. **Convert Referrals**
   - Close 60-80% of referred leads
   - Prioritize (highest close rate)
   - Thank referring client

3. **Support Cross-Sell Closes**
   - Help AMs with proposals
   - Jump in for complex deals
   - Close support

**Success Metrics:**
- Martal SQL → Close: 30-40%
- Referral → Close: 60-80%
- Overall new clients: 16-20
- Pipeline coverage: 3-4x monthly target

---

### Marketing Team

**Primary Responsibilities:**
1. **Martal Partnership Management**
   - Monthly performance reviews
   - Messaging optimization
   - ICP refinement

2. **Clutch Profile Management**
   - Track reviews
   - Respond to reviews
   - Optimize profile

3. **Referral Program Materials**
   - One-pagers
   - Email templates
   - Success stories

4. **Inbound Lead Gen**
   - Website optimization
   - SEO for Clutch
   - Content creation

**Success Metrics:**
- Martal SQL quality: >50% convert to opportunities
- Clutch reviews: 10-15 collected
- Clutch rating: 4.8+ average
- Inbound leads: 2-3/month

---

### Delivery Teams (Dev, Marketing, Consulting)

**Primary Responsibilities:**
1. **Quality Delivery**
   - Meet/exceed client expectations
   - Create review-worthy results
   - Build referral-worthy relationships

2. **Cross-Sell Support**
   - Support AMs in pitching services
   - Deliver on cross-sell promises
   - Ensure smooth multi-service delivery

3. **Client Retention**
   - Proactive communication
   - Anticipate issues
   - Go above and beyond

**Success Metrics:**
- Client NPS: 8+
- Cross-sell delivery quality: No churn within 90 days
- On-time delivery: 90%+
- Client retention: 75%+

---

### Operations Team

**Primary Responsibilities:**
1. **CRM & Tracking Setup**
   - Credit tracking system
   - Commission tracking
   - Program metrics dashboard

2. **Process Implementation**
   - QBR scheduling and templates
   - Renewal workflows
   - Client health monitoring

3. **Reporting**
   - Weekly metrics reports
   - Monthly program performance
   - Client health dashboards

**Success Metrics:**
- All programs tracked accurately
- Credits/commissions processed on time
- Reports delivered weekly
- Zero tracking errors

---

## Timeline & Milestones

### December 2025: Internal Preparation

**Week of Dec 2:**
- Finalize all program terms
- Set up CRM tracking
- Prepare training materials

**Week of Dec 9:**
- **Team announcement** (you're reading it!)
- Training session (Dec 11, 2:00 PM)
- Each AM identifies targets

**Week of Dec 16:**
- Review targets with leadership
- Begin renewal outreach for Q4 expiring contracts
- Prep for January launch

---

### January 2026: Soft Launch

**Week of Jan 6:**
- Soft launch to top 10 clients (referral + review)
- AMs begin Priority 1 cross-sell conversations
- Monitor initial results

**Week of Jan 13:**
- Track early wins
- Refine approach
- Prepare for full launch
- **Martal pilot results review**

---

### Late January - February: Full Launch

**Week of Jan 20:**
- Full launch email to ALL clients
- Add programs to email signatures
- Update client onboarding materials

**Week of Feb 3:**
- Review first month results
- Celebrate early wins
- Plan Q1 reminder campaigns

---

### March - December: Execution & Iteration

**Monthly:**
- Track all metrics
- Process credits/commissions
- Share success stories
- Pipeline reviews

**Quarterly:**
- Performance reviews
- Strategy adjustments
- Recognize top performers
- Reminder campaigns to clients

---

## Success Metrics: How We'll Track Progress

### Company-Level Metrics (Reported Weekly)

- **MRR:** Track weekly (target: $381k by Dec 31)
- **Active Clients:** Track monthly (target: 43-48 by Dec 31)
- **Churn Rate:** Track monthly (target: <25%)
- **New MRR Added:** Track monthly (target: $17.5k/month average)

---

### Program-Level Metrics (Reported Monthly)

**Cross-Sell Program:**
- Conversations initiated
- Proposals sent
- Cross-sells closed
- MRR added
- Commissions paid

**Referral Program:**
- Referrals made
- Referrals in pipeline
- Referrals closed
- Credits issued

**Review Program:**
- Reviews requested
- Reviews submitted
- Reviews published
- Average star rating

**Martal Program:**
- SQLs delivered
- **Migration SQLs delivered** (critical for Premier tier)
- SQLs → Opportunities
- Opportunities → Closes
- **Plus migrations closed** (target: 4 in 2026)
- MRR from Martal

**Shopify Premier Partner Status:**
- **Plus migrations closed quarterly** (target: 1 in Q1, 2 in Q2, 1 in Q3)
- **Shopify certifications completed** (target: 30 by June 30)
  - Q1 target: 22-25 certifications (12 current + 10-13 completed)
  - Q2 target: 30+ certifications
- Pipeline of migration opportunities (BigCommerce, Magento, WooCommerce)

---

### Individual/Team Metrics

**Account Managers:**
- Cross-sells pitched/closed
- Referrals generated
- Reviews collected
- Client retention rate
- Commissions earned

**Sales Team:**
- SQLs converted
- Referrals converted
- Overall closes
- Win rate

**Marketing:**
- Clutch reviews collected
- Martal SQL quality
- Inbound leads generated

**All Team Members (with assigned Shopify courses):**
- Shopify certifications completed (individual tracking)
- Target: Complete all assigned courses by March 31
- Company-wide target: 30 certifications by June 30

---

## What You Need to Succeed

### Training & Support

**December 11 Training Session (2:00 PM - 90 min)**
- Cross-sell training (pitch scripts, objection handling)
- Referral ask training (conversation starters)
- Review ask training (when and how)
- CRM tracking process
- Q&A

**Ongoing Support:**
- Weekly office hours with leadership
- Slack channel for program questions
- Templates and scripts provided
- One-on-one coaching as needed

---

### Resources & Tools

**You'll Get:**
- Cross-sell strategy guide (targets, approach)
- Client referral program one-pager
- Email templates for asking
- CRM tracking system
- Conversation scripts
- Objection handling guide
- Case studies and success stories

**Marketing Will Provide:**
- Referral program materials
- Clutch review links
- Social proof content
- Success story templates

---

### Compensation & Recognition

**Cross-Sell Commissions:**
- 5% of contract term value
- Paid 50% at signing, 50% after 90 days
- Average AM potential: $5-10k/year
- Top performers: $15k+

**Recognition:**
- Monthly shoutouts for top performers
- Quarterly awards
- Public recognition in team meetings
- Annual recognition for year-end leaders

**NO cash for referral/review asks** — those are client credits, but you'll be recognized for driving growth.

---

## Q1 2026: The Critical Quarter

**Why Q1 is Make-or-Break:**
- $60k in projects ending (but Satechi converting to retainer April!) ✅
- $39k in contracts expiring (Dec expirations need renewal focus)
- Need to close 7-9 new clients (aggressive but achievable)
- **Shopify certifications:** Must complete 10-13 in-progress courses to stay on track for Premier tier
- Sets momentum for entire year

**Recent Wins Going Into Q1:**
- ✅ Poseidon Bikes renewed for 12 months (10/28/25)
- ✅ Align Tech renewed for 6 months (9/25/25)
- ✅ Satechi confirmed converting to retainer 4/1/26
- ✅ 12 Shopify certifications completed (40% to Premier goal)
- **$23k MRR risk removed** - Q1 is more achievable than originally forecast!

**Q1 Critical Actions:**

**Week 1-2 (Late November/Early December):**
- [ ] Launch cross-sell, referral, and review programs
- [ ] **All team members: Begin completing assigned Shopify certifications**
- [ ] Sales: Re-engage Jerome's Furniture migration opportunity
- [ ] Sales: Schedule Bolle Brands check-in (Q2 migration)

**January:**
- [ ] First Martal SQLs arrive - prioritize migration opportunities
- [ ] Account managers: Identify top 3 cross-sell opportunities each
- [ ] **Dev team: Complete 5 remaining Liquid Storefronts certifications (HIGH PRIORITY)**
- [ ] **PM/Sales team: Complete 4 remaining Selling Fundamentals certifications (HIGH PRIORITY)**

**February:**
- [ ] Close 1st Plus migration (Q1 target)
- [ ] First cross-sells close
- [ ] First referrals submitted
- [ ] First Clutch reviews go live
- [ ] **Marketing team: Complete Marketing Fundamentals certifications (3 people)**

**March:**
- [ ] Push to close Q1 deals
- [ ] Review Martal pilot results (renew decision)
- [ ] **Complete remaining in-progress certifications → 22-25 total by March 31**

**If Q1 fails:**
- Start Q2 behind target
- Harder to catch up
- Morale impact
- **Risk losing Premier tier if certifications fall behind schedule**

**If Q1 succeeds:**
- Momentum builds
- Team confidence
- Easier path to year-end goal
- **On track for 30+ certifications by June 30** ✅

**All hands on deck for Q1.**

---

## Addressing the Elephant in the Room

### "Why should we believe this will work?"

**Fair question. Here's why:**

1. **Multiple channels reduce risk**
   - Not relying on one thing (Martal)
   - If one underperforms, others compensate

2. **All programs are performance-based**
   - We only pay when results happen
   - Low risk, high reward

3. **We're leveraging existing assets**
   - 30 active clients underutilized
   - Referrals and cross-sells tap into that

4. **Proven models**
   - Referrals: 60-80% close rate (industry standard)
   - Cross-sells: Multi-service clients have 0% loss rate vs. 46% for single-service
   - Project→Retainer: Satechi converting (our model works!)
   - Reviews: Standard agency growth tactic

5. **We have a plan AND resources**
   - Not winging it
   - $107-130k investment approved
   - Training and support provided

---

### "What if we miss the target?"

**Honest answer:** If we execute and still miss, at least we'll know we tried everything.

**But here's the reality:** If we do NOTHING different in 2026, we'll continue declining. That puts us at $3.2-3.3M by end of 2026, with cash flow increasingly tight.

**This plan gives us a fighting chance.**

And even if we hit $3.9M instead of $4.2M, that's still better than continued decline.

---

## What We Need From You

### 1. **Belief and Commitment**

This only works if everyone is bought in. We need:
- Active participation (not passive)
- Positive energy (not cynicism)
- Team mentality (not siloed)

---

### 2. **Focus on Execution**

Ideas don't drive revenue. Execution does.
- Follow the process
- Track your metrics
- Ask for help when stuck
- Iterate and improve

---

### 3. **Urgency**

We don't have time to waste. Q1 sets the tone.
- Move fast
- Don't overthink
- Action > perfection

---

### 4. **Accountability**

Own your role in the strategy.
- AMs: Cross-sells, referrals, reviews
- Sales: Close rates
- Marketing: Program support
- Delivery: Quality and retention

**We'll track metrics publicly. Everyone will see who's driving results.**

---

## The Upside

**If we hit our targets:**
- $4.2M revenue (vs. $3.2M in 2025)
- 46% MRR growth
- Reverse 3-year decline
- Stronger client base (43-48 clients vs. 30)
- Diversified lead sources (less feast/famine)
- Team morale boost
- Bonuses and commissions for all

**For you personally:**
- Job security (growing company vs. declining)
- Career growth (more opportunities)
- Commission potential (AMs)
- Pride in being part of turnaround

---

## Information Sharing Guidelines

### What to Share at Monthly Town Halls (Entire Team)

**DO share these metrics and updates:**

✅ **Performance against goals (percentages, not dollars):**
- "We're at 85% of our MRR goal for the month"
- "We've closed 3 new clients this quarter toward our target of 8-10"
- "We're tracking toward our annual growth target"

✅ **Client wins and losses (counts, not dollars):**
- "We added 2 new clients this month: [Client A] and [Client B]"
- "We lost 1 client to budget cuts"
- "We're at 32 active clients (up from 30 in November)"

✅ **Program launches and participation:**
- "The referral program is now live — ask your account managers how to participate"
- "We've received 5 Clutch reviews this quarter (target: 10-15)"
- "3 successful cross-sells completed in Q1"

✅ **Strategic initiatives (without financial details):**
- "We're working with Martal to generate qualified leads"
- "We're focusing on Shopify Plus projects to maintain our partner status"
- "We're targeting improved retention through QBRs"

✅ **Team recognition:**
- "Sarah landed a $7.5k/month cross-sell with Great Star Tools"
- "Mike generated 2 referrals that converted to new clients"
- "The team delivered 12 Clutch reviews this quarter"

✅ **What's working and what's not:**
- "Referrals are converting at 70% — keep them coming"
- "We need to improve our renewal outreach process"
- "Martal is delivering quality leads; sales team is closing them well"

---

### What to Keep in Management Meetings Only

❌ **DO NOT share at town halls:**

- **Specific revenue figures:** "$4.2M annual revenue," "$292k MRR"
- **Specific MRR targets:** "We need $381k MRR by year-end"
- **Client-level financials:** "AG Jeans pays us $15k/month"
- **Investment amounts:** "We're spending $72k/year on Martal"
- **Profit margins (absolute dollars):** "We're making $500k profit"
- **Individual client risk assessments:** "Wyze might reduce spend"
- **Churn rates (percentages):** "35% annual churn" (just say "higher than we'd like")
- **Historical revenue decline details:** "Down $1.66M over 3 years" (just say "recovering from recent decline")

---

### Monthly Town Hall Talking Points Template

Use this framework for monthly town halls:

**1. Performance Update (2 minutes):**
- "We're at [X%] of our MRR goal for [month]"
- "We're tracking [ahead/behind/on pace] for our annual targets"
- "Our profit margin is at [X%] this month"

**2. Client Activity (2 minutes):**
- "We added [X] new clients: [names]"
- "We're now at [X] active clients (up/down from last month)"
- "Notable wins: [mention any big clients or milestones]"

**3. Program Updates (2 minutes):**
- "Referral program: [X] referrals received, [X] converted"
- "Clutch reviews: [X] completed, [X] to go toward our goal"
- "Cross-sells: [X] successful this quarter"

**4. Challenges and Adjustments (1 minute):**
- "We lost [X] clients this month due to [budget cuts/project completion]"
- "We're adjusting our approach to [specific challenge]"

**5. Recognition (2 minutes):**
- Celebrate specific wins and contributors
- Recognize top performers

**6. Looking Ahead (1 minute):**
- Preview next month's priorities
- Any upcoming training or initiatives

**Total time: ~10 minutes of company-wide metrics update**

---

### Monthly Management Meeting Agenda

Use management meetings (not town halls) to discuss:

1. **Full Financial Review (10 minutes):**
   - Actual MRR vs. target MRR
   - Revenue projections and forecasts
   - Profit margin analysis
   - Budget vs. actual spend

2. **Client Health and Risk Assessment (10 minutes):**
   - At-risk clients and mitigation plans
   - Renewal pipeline and contract expirations
   - Client-level financial performance

3. **Strategic Initiatives Performance (10 minutes):**
   - Martal SQL count, close rate, cost per acquisition
   - Referral and cross-sell pipeline with dollar values
   - Program ROI analysis

4. **Resource Allocation and Investment Decisions (5 minutes):**
   - Budget approvals
   - Hiring/contractor needs
   - Program renewals (e.g., Martal)

5. **Operational Challenges and Problem-Solving (10 minutes):**
   - Bottlenecks and process improvements
   - Team capacity and workload
   - Cross-functional coordination

**Total time: ~45 minutes monthly**

---

## Final Thoughts

**We're not going to sugarcoat it:** 2025 was rough. Three years of decline from $4.88M to $3.6M has put us in a tough spot. The good news: the decline slowed from -20% to -6.5%, and profit margins improved from 5.6% to 6.0%.

**But we're not giving up.** We have a plan, we have resources, and we have a team capable of executing.

**2026 is our turnaround year.** It won't be easy. It'll require focus, urgency, and teamwork.

**But we can do this.**

Let's go make it happen.

---

**Next Steps:**

1. **Read this document fully** (you just did!)
2. **Attend training session** (Dec 11, 2:00 PM)
3. **Prepare your targets** (AMs: top 3 cross-sells, top 5 referrers, top 3-5 reviewers)
4. **Ask questions** (Slack, email, 1-on-1s)
5. **Get ready to execute** (January launch)

---

**Prepared by:** Tim Katz
**Date:** November 2025
**Next Update:** January 2026 (Q1 kickoff)

---

## Appendix: FAQ

**Q: What if a client says no to a cross-sell?**
A: That's fine. Don't push. Move on to next target. Not every client is a fit.

**Q: How do I know which clients to ask for referrals?**
A: Happy clients, well-connected, engaged. We'll help you identify them in training.

**Q: What if Martal fails?**
A: We'll pivot. Referrals and cross-sells compensate. May hire in-house sales exec.

**Q: Can I earn commission on referrals?**
A: No, referral credits go to the CLIENT (not you). But you get recognized for driving them.

**Q: What if I don't hit my cross-sell target?**
A: No penalty. It's a bonus opportunity, not a requirement. But top performers will be recognized.

**Q: When do commissions get paid?**
A: 50% at contract signing, 50% after 90-day retention milestone.

**Q: What if a cross-sold client churns within 90 days?**
A: Commission is clawed back (50% returned). Focus on quality fits.

**Q: How much time will this take?**
A: Cross-sells: 2-3 hours per pitch. Referral asks: 5 minutes in QBR. Review asks: 5 minutes after success.

**More questions? Ask in training or Slack!**
